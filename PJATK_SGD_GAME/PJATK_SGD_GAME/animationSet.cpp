...
AnimationSet::~AnimationSet() {
	cleanup(spriteSheet);
	if (whiteSpriteSheet != NULL) //prevents memory leaks
...
Animation* AnimationSet::getAnimation(string name) {
	for (list<Animation>::iterator animation = animations.begin(); animation != animations.end(); animation++) {
		Animation* anim = &(*animation); //get memory adress of the i point

		//if we find a match on name, return the animion from the list
		if (name == anim->name) {
			return anim;
		}
	}
