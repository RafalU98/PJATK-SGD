...
Frame* Animation::getFrame(int frameNumber) {
	...
	list<Frame>::iterator i = frames.begin(); //point iterator to first frame in list

	int counter = 0;

	for (counter = 0; counter < frameNumber && counter < frames.size() - 1; counter++) {
		i++; // make iterator point the next frame in list
	}

	Frame* frame = &(*i); //make frame point to the frame inside the list from iterator 
						// *i = gets frame at i position (de-referencing)
						// & = get the memory adress  of i (referencing)
