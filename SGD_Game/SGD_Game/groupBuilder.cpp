...
void GroupBuilder::buildGroups(std::list<DataGroupType> groupTypes, std::list<Group*> &groups){
	for (std::list<DataGroupType>::iterator dgt = groupTypes.begin(); dgt != groupTypes.end(); dgt++)
	{
...
Group* GroupBuilder::findGroupByName(std::string str, std::list<Group*> &groups){
	for (std::list<Group*>::iterator group = groups.begin(); group != groups.end(); group++)
	{
